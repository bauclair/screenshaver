...
-- playlists
-- ---------------------------------------------------------------------------
-- One row represents one user-defined ordered collection of shader policies.
-- A playlist may contain screensaver, wallpaper, and unassigned policies at
-- the same time. Policy target remains a property of shader_policies.
CREATE TABLE playlists (
    ...
    playlist_name          TEXT NOT NULL
                           CHECK (
                               length(trim(playlist_name)) BETWEEN 1 AND 128
                           ),

    -- Unicode-normalized, case-folded comparison key generated by Rust using
    -- the same canonical algorithm used for shader_policies.policy_name_key.
    -- Playlist Names are case-insensitively unique.
    playlist_name_key      TEXT NOT NULL
                           UNIQUE
                           CHECK (length(playlist_name_key) > 0),

    -- Optional free-form user description.
    description            TEXT
);
...
-- playlist_members
-- ---------------------------------------------------------------------------
-- Ordered many-to-many relationship between playlists and shader policies.
-- position is the authoritative persistent playlist/playback order.
-- Screensaver and wallpaper playback filter this common ordering by the
-- current shader_policies.policy_target without maintaining separate orders.
CREATE TABLE playlist_members (
    ...
    PRIMARY KEY (playlist_id, policy_id),
...
-- Reverse membership lookup used by QBE and Policy context operations:
--   "Which playlists contain this policy?"
CREATE INDEX idx_playlist_members_policy_id
    ON playlist_members(policy_id);
...
CREATE TABLE runtime_targets (
    ...
    -- Rotation interval for ordered/random/playlist modes.
    ...
    -- NULL for ordered/random/playlist modes. ON DELETE SET NULL deliberately
    -- leaves Single mode recoverable if its selected policy is deleted.
    single_policy_id        INTEGER,

    -- Exact playlist selected for Playlist mode.
    -- NULL for single/ordered/random modes. ON DELETE SET NULL deliberately
    -- leaves Playlist mode recoverable if its selected playlist is deleted.
    playlist_id             INTEGER,
...
--      playlist_id is NULL for both initial rows. No user playlists or
--      playlist memberships are created during factory initialization.
--
