    ...
    class extends Shell.GLSLEffect {
        ...
        vfunc_paint_target(node, paintContext) {
            ...
            try {
                ...
                // GNOME 46 compatibility: when the Cogl.Context is obtained
                // from the PaintContext framebuffer, use standard RGBA_8888
                // targets only.  Full-size RGBA_8888 allocation has been
                // validated safely; the production FP16 path caused a native
                // GNOME Shell crash before downstream Test #30 logging.
                const forceStandardPrecision =
                    coglContextSource === 'Clutter PaintContext framebuffer';

                if (forceStandardPrecision &&
                    !this._screenshaverStandardPrecisionFallbackLogged) {
                    console.log(
                        `[Screenshaver] Test #30J GNOME paint-context production fallback: ` +
                        `forcing color precision=standard (RGBA_8888) ` +
                        `generation=${generation}`
                    );
                    this._screenshaverStandardPrecisionFallbackLogged = true;
                }
                ...
                if (needsRenderTarget) {
                    ...
                    if (forceStandardPrecision ||
                        ...
                        if (forceStandardPrecision &&
                            ...
                            fellBack = true;
                            fallbackReason =
                                'GNOME paint-framebuffer context requires standard RGBA_8888';
                        }
