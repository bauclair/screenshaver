...
pub(crate) struct FrameRenderEngine {
    ...
    audio_motion_state: crate::render_audio_motion::AudioMotionState,
    audio_motion_lyrics_manager: Option<crate::manage_lyrics::LyricsManager>,
                ...
                match crate::manage_lyrics::LyricsManager::start() {
                    ...
                    Err(error) => {
                        crate::logger::warning(
                            ...
                            &format!("[AUDIO_MOTION] LRCMUX timing unavailable; using ungated spectrum: {}", error),
                        );
                        None
                    }
                }
...
// Selection alone must not advance persistent Ordered state. At this
        ...
        Ok(
            Self {
                ...
                audio_bands,
                audio_required,
                audio_motion_effect: postprocess_profile.audio_motion,
                audio_motion_state: crate::render_audio_motion::AudioMotionState::default(),
    ...
    pub(crate) fn render_frame(
        ...
        self.render_frame_to_framebuffer(
            width,
            height,
            0,
        )
    ...
    /// Render one frame into a framebuffer owned by the active presentation
    /// host. The normal SDL path continues to call `render_frame()` and uses
    /// framebuffer zero. Embedded hosts such as KDE/Qt Quick pass the
    /// scene-graph framebuffer that was active when the render node began.
    pub(crate) fn render_frame_to_framebuffer(
        &mut self,
        width: u32,
        height: u32,
        output_framebuffer: u32,
