...
pub fn run() -> Result<(), String> {
    ...
    let finder = PlayerFinder::new().map_err(|error| {
        format!("Unable to connect to the session D-Bus for MPRIS discovery: {}", error)
    })?;

    let players = finder.find_all().map_err(|error| {
        format!("Unable to enumerate MPRIS media players: {}", error)
    })?;

    if players.is_empty() {
        println!("No MPRIS media players are currently available.");
        println!("Start Kew and play or pause a track, then run --test-lyrics again.");
        return Ok(());
    }
    ...
    let player = players
        .iter()
        .find(|player| {
            player.bus_name_trimmed().eq_ignore_ascii_case("kew")
                || player.identity().eq_ignore_ascii_case("kew")
        })
        .unwrap_or(&players[0]);

    println!("Selected MPRIS player: {}", player.identity());
    println!("Bus name:              {}", player.bus_name());
    println!();

    let mut current_track = read_track_information(player)?;
    print_track_information(&current_track);
    println!();
    println!("Querying LRCLIB...");
    println!();

    let mut synchronized_lines = match retrieve_synchronized_lines(&current_track) {
        Ok(lines) => lines,
        Err(error) => {
    ...
    run_graphical_test(
        player,
        &mut current_track,
        &mut synchronized_lines,
    )
}

fn run_graphical_test(
    player: &Player,
    current_track: &mut TrackInformation,
    synchronized_lines: &mut Vec<SynchronizedLine>,
) -> Result<(), String> {
    let sdl = sdl2::init().map_err(|error| format!("SDL initialization failed: {}", error))?;
    let video = sdl.video().map_err(|error| format!("SDL video initialization failed: {}", error))?;
    ...
    'render: loop {
        ...
        if last_metadata_poll.elapsed() >= METADATA_POLL_INTERVAL {
            ...
            match read_track_information(player) {
                Ok(observed_track) => {
                    if observed_track.identity != current_track.identity {
                        println!();
                        println!("Track change detected.");
                        println!();
                        ...
                        *current_track = observed_track;
                        *synchronized_lines = match retrieve_synchronized_lines(current_track) {
                            Ok(lines) => lines,
                            Err(error) => {
                                eprintln!("[LYRICS TEST] {}", error);
                                Vec::new()
                ...
                Err(error) => eprintln!("[LYRICS TEST] Unable to refresh track metadata: {}", error),
            }
        }
        ...
        if last_position_poll.elapsed() >= POSITION_POLL_INTERVAL {
            ...
            match player.get_playback_status() {
                ...
                Err(error) => eprintln!("[LYRICS TEST] Unable to read playback status: {}", error),
            }
        }
        ...
        unsafe {
            ...
            if scene_resolution >= 0 {
                gl::Uniform2f(scene_resolution, drawable_width as f32, drawable_height as f32);
            }
            gl::BindVertexArray(vao);
...
fn retrieve_synchronized_lines(
    ...
    let synchronized_lines =
        parse_synchronized_lyrics(
            synced_lyrics
        )?;
